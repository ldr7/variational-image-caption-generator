...
def run(args):
    ...
    while True:
        ...
        print("Training : Mean G loss : {} / Mean D loss : {} ({} seconds elapsed)".format(g_loss/g_batch, d_loss/d_batch, time.time()-secs),flush=True)
        ...
        print("BLEU-{} score : {}".format(max_bleu, score), flush = True)
        ...
        print("Best BLEU so far : {} (Epoch {})".format(best_bleu[0], best_bleu[1]),flush=True)
        ...
        model.train(True)
        torch.set_grad_enabled(True)
        print("Epoch finished in {} seconds".format(time.time()-secs),flush=True)

        if epoch - best_bleu[1] == 3 and epoch>5:
        #if epoch>15:
            break
    ...
    if logging:
        ...
        print("Scores saved in {}".format(output_scores),flush=True)
